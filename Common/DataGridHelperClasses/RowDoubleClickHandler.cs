...
namespace CommonFrontEnd.Common.DataGridHelperClasses
{
        ...
        public RowDoubleClickHandler(DataGrid dataGrid)
        {
            MouseButtonEventHandler handler = (sender, args) =>
            {
                ...
                if (row != null && row.IsSelected)
                {
                    ...
                    var method = dataContextType.GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
                    if (method == null)
                    {
